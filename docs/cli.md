...
cache hit 후에는 기본적으로 상위 후보 pool을 WZ에서 다시 열어 pixel-level score로 refine합니다. cache-hit scoring은 구조 점수 상한으로 최종 top-K를 넘을 수 없는 후보의 color/shape 계산을 생략합니다. 결과 JSON에는 `StructuralScore`, `DHashDistance`, `EdgeHashDistance`, `CoarsePrefilteredImageCount`가 포함됩니다. `--probe`는 `--trust-cache --no-refine`의 shorthand이며 빠른 1차 후보 탐색용입니다. 기본 검색보다 정확도가 낮을 수 있으므로 최종 확정에는 refine가 켜진 기본 검색을 사용합니다.
