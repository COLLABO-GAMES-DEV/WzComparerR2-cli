...
- `skill full`은 이제 CLI repository/find service를 통해 입력 후보를 관리합니다. positional skill 입력, `--skill-wz`, `--data-dir/Skill`을 data 후보로 보고, `--string-wz`, `--data-dir/String`, positional `Data/Skill`의 sibling `Data/String`을 string 후보로 자동 추가합니다. 출력에는 실제 hit 경로 `DataInputPath`, `StringInputPath`와 전체 후보 목록이 포함됩니다.
