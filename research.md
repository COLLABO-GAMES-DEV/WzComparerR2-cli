...
- 2026-09-04 `3141000` 폭풍의 시 VI의 `prepare`/`keydown`/`keydownend` 흔들림 원인을 조사했습니다. 기존 export는 `Data/Skill` visual-only node를 직접 추출해 PNG SHA는 정상이었지만 `Origin`/`Delay`/`Z`가 모두 `null`이었습니다. 원본 metadata는 `Data/Packs/Skill_00004.ms :: Skill/314.img/skill/3141000`에 있었고, 각 frame은 `Skill/_Canvas/314.img/skill/3141000/...` `_outlink`와 `origin`/`delay`/`z`를 갖고 있었습니다. CLI가 같은 pack 안의 `312.img/skill/3120049/psdSkill/3141000` marker를 먼저 잡거나 `Data/Skill` visual-only 후보에서 멈추지 않도록, `DomainInfoFinder`와 `CliWzRepository`를 보정했습니다. 재검증 산출물은 `.test/wcr2-skill-3141000-metadata-fixed-20260904-073424`이며, PNG SHA는 기존 export와 동일하고 `resources.json`에는 metadata `InputPath`, `MetadataSourcePath`, canvas `ResolvedInputPath`, branch-level `action`/`time`/`repeat`, frame-level `Origin`/`Delay`/`Z`가 기록됩니다. Character `Character.wz`, `Character_000.wz`, `Character/_Canvas`에서는 `6thStormArrow` 이름/값 검색 결과가 없었으므로 현재 데이터 기준으로는 별도 Character action sprite mapping이 아니라 Skill pack metadata + Skill/_Canvas outlink 결합 케이스입니다.
- 2026-09-07 map metadata 검증에서 split map directory `Data/Map/Map/Map1` 안에는 `_Canvas/100000000.img` preview와 실제 `100000000.img` metadata가 같은 이름으로 공존했습니다. 기존 `map portals/life/objects/reactors`는 `_Canvas` 후보를 먼저 잡아 exit 0이면서 빈 배열을 반환할 수 있었습니다. `DomainInfoFinder`는 이제 map 검색 시 `_Canvas` 경로를 preferred 후보에서 제외하고, `portal`, `life`, `reactor`, layer `obj/tile`이 있는 populated node를 우선합니다. 재검증 산출물은 `.test/wcr2-map-detail-smoke-20260907`이며, 헤네시스 `100000000`에서 portals 37개, life 35개, objects/tiles 1603개, reactors 0개를 확인했습니다.
- 2026-09-07 Agent Runtime에 `skill.export-xlsx` step을 추가했습니다. 이 step은 OpenXML `.xlsx` 파일을 외부 NuGet 없이 zip/xml로 읽고, header에서 `직업`, `직업 코드`, `스킬`/`skill` 열을 자동 감지해 중간 `names.tsv`를 만든 뒤 기존 `skill.export-batch` Headless 경로를 재사용합니다. 표 구조가 다르면 `jobNameColumn`, `jobCodeColumn`, `skillColumns`, `headerRow`, `firstDataRow`를 명시할 수 있습니다. 기본 output pattern은 `{jobCode}_{jobName}/{id}_{name}`이고, recipe sidecar는 `_agent/<stepId>-xlsx-recipe.json`에 남습니다. 검증 산출물은 `.test/wcr2-agent-xlsx-skill-export-20260907/rerun4`이며, `보우마스터/314/폭풍의 시 VI`가 `314_보우마스터/3141000_폭풍의 시 VI`로 추출됐습니다.
- 2026-09-07 Agent Runtime Phase 5에서 `item.icon`, `item.export`, `map.export` step을 추가했습니다. `item.icon` 기존 구현은 `WzComparerR2.Headless/Items`로 이동해 CLI와 AgentHost가 같은 로직을 씁니다. `item.export`는 item metadata와 icon result를 묶어 `item-info.json`, `item-icon-result.json`, `agent-item-export-result.json`을 저장합니다. `map.export`는 렌더가 아니라 metadata export로 `map-info.json`, `map-metadata.json`, `agent-map-export-result.json`을 저장합니다. 검증 산출물은 `.test/wcr2-agent-item-map-20260907`이며, `미라클 큐브` icon 1개, `빨간 포션` item export icon 1개, `헤네시스` portals 37/life 35/objects 1603/reactors 0을 확인했습니다.
- 2026-09-07 Agent Runtime Phase 6 기본 `serve --stdio`를 추가했습니다. stdin/stdout newline-delimited JSON으로 `ping`, `run`, `shutdown`을 처리하고, `id`/`requestId`를 response `id`에 보존합니다. `run`은 기존 `AgentJobRunner`를 호출하며 `jobPath` 또는 inline `job` object를 받을 수 있습니다. response는 한 줄 compact JSON입니다. 검증은 `WzComparerR2.Cli.Tests`의 `agent serve stdio handles ping run shutdown`으로 추가했고, 현재 request 사이의 WZ repository/session cache는 아직 미구현입니다.
