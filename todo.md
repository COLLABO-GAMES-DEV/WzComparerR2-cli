...
- [x] `image search` 유사 이미지 검색 명령을 추가한다.
  ...
  - [x] 2026-09-09 image search runtime bucket index: cache v3 파일 포맷은 유지하고, 로드된 cache를 width/height/aspect/alpha bucket으로 묶어 size/shape가 맞지 않는 bucket을 건너뛴다. 결과 JSON에는 `BucketPrefilteredImageCount`, `ScoringBucketCount`가 포함된다. mob query 기준 이전 v3 top 30 diff 없음, cache-hit refine 3.17초, probe 1.26초.
